...
function JavaScript_Konfirmasi_Itemrevisi( id, iditem ){
	var answer = confirm ("Are you sure want to delete this file.")
	if (answer)
	window.location="produk_proses_itemrevisi.php?action=itemrevisi_hapusdata&iditem="+  iditem  +"&id=" + id 
	else
	window.location="produk_item_revisi.php?iditem=" + iditem
}
...
function JavaScript_Konfirmasi_Itemlogakses( id ){
	var answer = confirm ("Are you sure want to delete this file.")
	if (answer)
	window.location="produk_proses_itemlogakses.php?action=itemrevisi_hapusdata&id=" + id 
	else
	window.location="produk_item_itemlogakses.php?id=" + id
}
...
$detail_subkanal = Select_Detail_KategoriSub_produk( $tbl_produkkategorisub, $idkategorisub )
...
<span class="JudulKanal_box1"> PRODUK | halaman  DATA <?php
...
/* Form Komentar */
	$FormprodukItemKomentar_Action = "produk_proses_itemkomentar.php?action=itemkomentar_tambahdata";
	$produkkategori_submitbutton = "Tambah Komentar...";
	$Tombol_CANCEL = "<input type='reset' name='reset' value='CANCEL' class='button' onClick=\"javascript:location.replace('produk_item_komentar.php?iditem=$iditem')\" >";
	...
	switch ($action){
		...
		<tr class='kontenform'>
		<td>";
		include"produk_detail_item.php";
		$iditem = $id;
		echo "<br>";
		include"produk_list_itemlampiran.php";
		echo "
		</td>
		</tr>
		</table>
