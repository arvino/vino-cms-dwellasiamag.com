...
if( $keterangan=='' ){
	...
	header("location:produk_subkanal.php?idkategori=$idkategori&pesan_error=PLEASE FILL TITLE"); /* Kembali ke halaman  sebelumnya */
	
}else{

$id = produkKategoriSub_CreateID( $tbl_produkkategorisub ); 
$idupline = $idkategori;
...
$sql_hitungdata = produkKategoriSub_PeriksaJudul( $tbl_produkkategorisub, $idkategori, $keterangan, $keteranganinggris ); /* Periksa Judul Sub Kategori produk, Apakah masih tersedia. */
$hitung_data = mysql_num_rows($sql_hitungdata);
if( $hitung_data > 0 ){ /* Jika judul sudah pernah there is CANCELkan proses tambah data. */

	header("location:produk_subkanal.php?idkategori=$idkategori&pesan_error=FAILED ADD SUB SECTION  !,THE TITLE IS ALREADY EXIST !"); /* Kembali ke halaman  sebelumnya */
...
if( $image_file_logo == "" ){ /* JIka tida ada upload file logo */
	...
	$loc_root = "../";	
	$loc_file = "filemodul/produk/file_subkanal/";
...
/* Tambah data ke Database */
	...
	header("location:produk_subkanal.php?idkategori=$idkategori&pesan_error=SUCCESS ADD DATA !"); /* Kembali ke halaman  sebelumnya */
...
$idkategori = $_POST['idkategori'];
	if( $keterangan=='' ){
		header("location:produk_subkanal.php?idkategori=$idkategori&id=$id&action=EditData&pesan_error=PLEASE FILL THE TITLE"); /* Kembali ke halaman  sebelumnya */
	}else{ 
...
produkKategoriSub_UpdateData(
		...
		$hit, $linkjudul, $keyword 
	 	...
	 	header("location:produk_subkanal.php?idkategori=" . $idkategori . "&id=" . $id . "&action=EditData&pesan_error=UPDATE SUCCESSFULLY... !."); /* Kembali ke halaman  sebelumnya */
...
if ((!in_array($KodeKeamananhalaman , $halamandiizinkan) OR !isset($KodeKeamananhalaman ))) {
	header("location:produk_subkanal.php?pesan_error=FAILED ! ACCESS DENIED.");  
}else{
	$id = $_GET['id'];
	...
	produkKategoriSub_DeleteImageLogo( $tbl_produkkategorisub, $id );
	header("location:produk_subkanal.php?action=EditData&idkategori=$idkategori&id=$id&pesan_error=SUCCESS DELETE IMAGE DATA"); 
}
}
...
if ((!in_array($KodeKeamananhalaman , $halamandiizinkan) OR !isset($KodeKeamananhalaman ))) {
	...
	unlink($image_subkanal);
	produkKategoriSub_DeleteImageHeader( $tbl_produkkategorisub, $id );
	header("location:produk_subkanal.php?action=EditData&idkategori=$idkategori&id=$id&pesan_error=SUCCESS DELETE DATA."); 
}
}
...
if ((!in_array($KodeKeamananhalaman , $halamandiizinkan) OR !isset($KodeKeamananhalaman ))) {
	...
	unlink($image_subkanal);
	produkKategoriSub_DeleteImageBackground( $tbl_produkkategorisub, $id );
	header("location:produk_subkanal.php?action=EditData&idkategori=$idkategori&id=$id&pesan_error=SUCCESS DELETE IMAGE DATA"); 
}
}
/* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- */
...
if ((!in_array($KodeKeamananhalaman , $halamandiizinkan) OR !isset($KodeKeamananhalaman ))) {
	...
	header("location:produk_subkanal.php?pesan_error=SUCCESS PUBLISH DATA."); 
}
}
/* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- */
...
if( $action=="subkanal_hapusdata" ){ /* Proses Update Kanal*/
	...
	$image_subkanal = $root_image . $imagefile . $imagelogo;
	unlink($image_subkanal);
	produkKategoriSub_hapusdata( $tbl_produkkategorisub, $id );
	header("location:produk_subkanal.php?idkategori=" . $idkategori . "&pesan_error=SUCCESS DELETE DATA."); /* Kembali ke halaman  sebelumnya */
