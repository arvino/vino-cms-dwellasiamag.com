...
/* UPLOAD IMAGE THUMBNAIL FOR PREVIEW */
...
/* UPLOAD IMAGE MEDIUM FOR HEADLINE FRAME */
	$nama_file_medium = $_FILES['file_medium']['name'];
	$temp_file_medium = $_FILES['file_medium']['tmp_name']; 
	$ukuran_file_medium = $_FILES['file_medium']['size'];
	$tipe_file_medium = $_FILES['file_medium']['type'];
...
if( $file_medium == "" ){
	...
	$uploaddir = $location_dir;
	/* DELETE FILE LAMA  */
	$Delete_gambaritem_kecil = $loc_root . $direktorigambar . $gambarsedang;
	$new_name_gs =  $id . "_medium_";
	//$gambarkecil = cropImage( $new_name_gk, $uploaddir, $temp_file_thumb, $ukuran_file_thumb, $nama_file_thumb, $tipe_file_thumb, $nw=300, $nh=225 );
	$gambarsedang = Upload_File( $new_name_gs, $uploaddir, $temp_file_medium, $ukuran_file_medium, $nama_file_medium );

}


/* UPLOAD IMAGE BIG FOR DETAIL PAGE */
...
if( $file_thumb == "" ){
	...
	$uploaddir = $loc_root . $direktorigambar;
	/* DELETE FILE LAMA */
	$Delete_gambaritem_kecil = $loc_root . $direktorigambar . $gambarkecil;
	$new_name_gk =  $id . "_small_";
	/*$gambarkecil = cropImage( $new_name_gk, $uploaddir, $temp_file_thumb, $ukuran_file_thumb, $nama_file_thumb, $tipe_file_thumb, $nw=300, $nh=225 );*/
...
if( $file_medium == "" ){
	$gambarsedang = $_POST['gambarsedang'];
}else{
	$uploaddir = $loc_root . $direktorigambar;
	/* DELETE FILE LAMA  */
	$Delete_gambaritem_kecil = $loc_root . $direktorigambar . $gambarsedang;
...
/* UPLOAD IMAGE BIG FOR IMAGE DETAIL PAGE  */
	$nama_file = $_FILES['file_detail']['name'];
	$temp_file = $_FILES['file_detail']['tmp_name']; 
	$ukuran_file = $_FILES['file_detail']['size'];
	$tipe_file = $_FILES['file_detail']['type'];
...
if( $action=="item_listurutan" ){ /* Proses Update item*/
	$id = $_GET['id'];
	$urutan = $_GET['urutan'];
	newsitem_listurutan( $tbl_news,  $id, $urutan );
	header("location:news_main.php?pesan_error=SUCCESS UPDATE DATA"); /* Kembali ke halaman  sebelumnya */
}/* END ITEM Delete DATA */	
