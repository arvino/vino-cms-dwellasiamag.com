...
if( $keterangan=='' ){
	...
	header("location:peopledirectory_subkanal.php?idkategori=$idkategori&pesan_error=PLEASE FILL TITLE"); /* Kembali ke halaman  sebelumnya */
	
}else{

$id = peopledirectoryKategoriSub_CreateID( $tbl_peopledirectorykategorisub ); 
$idupline = $idkategori;
...
$sql_hitungdata = peopledirectoryKategoriSub_PeriksaJudul( $tbl_peopledirectorykategorisub, $idkategori, $keterangan, $keteranganinggris ); /* Periksa Judul Sub Kategori peopledirectory, Apakah masih tersedia. */
$hitung_data = mysql_num_rows($sql_hitungdata);
if( $hitung_data > 0 ){ /* Jika judul sudah pernah there is CANCELkan proses tambah data. */

	header("location:peopledirectory_subkanal.php?idkategori=$idkategori&pesan_error=FAILED ADD SUB SECTION  !,THE TITLE IS ALREADY EXIST !"); /* Kembali ke halaman  sebelumnya */
...
if( $image_file_header == "" ){ /* JIka tida ada upload file header */
	...
	$location_dir =  $loc_root . $loc_file;
		...
		$uploaddir = $location_dir;
		$new_name = "peopledirectory_img_header_" . $keterangan;
		$imagefile =  $loc_file;
...
if( $image_file_background == "" ){ /* JIka tida ada upload file background */
	...
	$location_dir =  $loc_root . $loc_file;
		...
		$uploaddir = $location_dir;
		$new_name = "peopledirectory_img_background_" . $keterangan;
		$imagefile =  $loc_file;
...
/* Tambah data ke Database */
	peopledirectoryKategoriSub_TambahData(
	...
	header("location:peopledirectory_subkanal.php?idkategori=$idkategori&pesan_error=SUCCESS ADD DATA !"); /* Kembali ke halaman  sebelumnya */
...
$idkategori = $_POST['idkategori'];
	if( $keterangan=='' ){
		header("location:peopledirectory_subkanal.php?idkategori=$idkategori&id=$id&action=EditData&pesan_error=PLEASE FILL TITLE"); /* Kembali ke halaman  sebelumnya */
	}else{ 
...
peopledirectoryKategoriSub_UpdateData(
		$tbl_peopledirectorykategorisub,
		$id, $idupline, $idkategori,
		$keterangan, $keteranganinggris,
		$statustampil, $menuatas1, $menuatas2,
		...
		$hit, $linkjudul, $keyword 
	 	...
	 	header("location:peopledirectory_subkanal.php?idkategori=" . $idkategori . "&id=" . $id . "&action=EditData&pesan_error=UPDATE SUCCESSFULLY... !."); /* Kembali ke halaman  sebelumnya */
...
if ((!in_array($KodeKeamananhalaman , $halamandiizinkan) OR !isset($KodeKeamananhalaman ))) {
	header("location:peopledirectory_subkanal.php?pesan_error=FAILED ! ACCESS DENIED.");  
}else{
	$id = $_GET['id'];
	$row_subkanal = Select_Detail_KategoriSub_peopledirectory( $tbl_peopledirectorykategorisub, $id );
	$root_image = "../";
	$id = $row_subkanal->id;
	...
	peopledirectoryKategoriSub_DeleteImageLogo( $tbl_peopledirectorykategorisub, $id );
	header("location:peopledirectory_subkanal.php?action=EditData&idkategori=$idkategori&id=$id&pesan_error=DELETE SUCCESSFULLY."); 
}
}
...
if ((!in_array($KodeKeamananhalaman , $halamandiizinkan) OR !isset($KodeKeamananhalaman ))) {
	...
	unlink($image_subkanal);
	peopledirectoryKategoriSub_DeleteImageHeader( $tbl_peopledirectorykategorisub, $id );
	header("location:peopledirectory_subkanal.php?action=EditData&idkategori=$idkategori&id=$id&pesan_error=DELETE SUCCESSFULLY."); 
}
}
...
if ((!in_array($KodeKeamananhalaman , $halamandiizinkan) OR !isset($KodeKeamananhalaman ))) {
	...
	unlink($image_subkanal);
	peopledirectoryKategoriSub_DeleteImageBackground( $tbl_peopledirectorykategorisub, $id );
	header("location:peopledirectory_subkanal.php?action=EditData&idkategori=$idkategori&id=$id&pesan_error=DELETE SUCCESSFULLY."); 
}
}
/* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- */
...
if ((!in_array($KodeKeamananhalaman , $halamandiizinkan) OR !isset($KodeKeamananhalaman ))) {
	...
	$row_kanal = Select_Detail_Kategori_peopledirectory( $tbl_peopledirectorykategori, $id );
	$idkategori = $row_kanal->id;
	peopledirectoryKategori_StatusTampil( $tbl_peopledirectorykategori, $statustampil, $id );
	header("location:peopledirectory_subkanal.php?pesan_error=SUCCESS PUBLISH DATA."); 
}
}
/* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- */
...
if ((!in_array($KodeKeamananhalaman , $halamandiizinkan) OR !isset($KodeKeamananhalaman ))) {
	...
	$row_kanal = Select_Detail_Kategori_peopledirectory( $tbl_peopledirectorykategori, $id );
	$idkategori = $row_kanal->id;
	peopledirectoryKategori_HomehalamanTampil( $tbl_peopledirectorykategori, $statustampil, $id );
	header("location:peopledirectory_subkanal.php?pesan_error=SUCCESS PUBLISH DATA"); 
}
}
/* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- */
...
if ((!in_array($KodeKeamananhalaman , $halamandiizinkan) OR !isset($KodeKeamananhalaman ))) {
	...
	$row_kanal = Select_Detail_Kategori_peopledirectory( $tbl_peopledirectorykategori, $id );
	$idkategori = $row_kanal->id;
	peopledirectoryKategori_menuatas1_tampil( $tbl_peopledirectorykategori, $statustampil, $id );
header("location:peopledirectory_subkanal.php?pesan_error=SUCCESS PUBLISH DATA");  
}
}
/* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- */
...
if ((!in_array($KodeKeamananhalaman , $halamandiizinkan) OR !isset($KodeKeamananhalaman ))) {
	...
	$row_kanal = Select_Detail_Kategori_peopledirectory( $tbl_peopledirectorykategori, $id );
	$idkategori = $row_kanal->id;
	peopledirectoryKategori_MenuTampil2( $tbl_peopledirectorykategori, $statustampil2, $id );
	header("location:peopledirectory_subkanal.php?pesan_error=SUCCESS PUBLISH DATA");  
}
}
/* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- *//* ---- */
...
if( $action=="subkanal_hapusdata" ){ /* Proses Update Kanal*/
	...
	$image_subkanal = $root_image . $imagefile . $imagelogo;
	unlink($image_subkanal);
	peopledirectoryKategoriSub_hapusdata( $tbl_peopledirectorykategorisub, $id );
	header("location:peopledirectory_subkanal.php?idkategori=" . $idkategori . "&pesan_error=SUCCESS DELETE DATA."); /* Kembali ke halaman  sebelumnya */
